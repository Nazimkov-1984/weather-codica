...
const CityCard: React.FC<IWeatherProps> = ({ data }): JSX.Element => {
  ...
  const updateWeather = useCallback((event: { preventDefault(): void }) => {
    event.preventDefault();
    dispatch(setLoading(data.id));
    dispatch(updateCity(data.name));
  }, [data.id, data.name, dispatch]);

  const deleteCity = useCallback((event: { preventDefault(): void }) => {
    event.preventDefault();
    dispatch(deletedCity(data.id));
  }, [data.id, dispatch]);

  const currentTemp: number = useMemo(() => Math.round(data.main.temp), [data.main.temp]);
  const currentTime: string = useMemo(() => {
    return new Date(data.dt * 1000).toLocaleTimeString("en-US", {
      hour: "2-digit",
      minute: "2-digit",
      hour12: true,
    });
