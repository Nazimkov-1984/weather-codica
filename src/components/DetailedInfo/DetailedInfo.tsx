...
import { IHourlyWeather, IWeatherDataDetailedInformation } from "../../redux/types";
import { getWeatherDetailedInformation } from "../../redux/actions";
import getTime from "../../servises/getTime";
...
const DetailedInfo: React.FC = (): JSX.Element => {
  ...
  const cityFullInformation:IWeatherDataDetailedInformation | null = useSelector(
    (state: RootState) => state.cityWeather.cityFullInformation
  );
  const loading:boolean = cityFullInformation?.hourly.length === 0;
  ...
  return (
    <Container data-testid="fdetailedinfo">
